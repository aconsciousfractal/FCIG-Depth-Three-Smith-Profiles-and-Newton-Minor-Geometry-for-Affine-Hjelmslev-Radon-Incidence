#!/usr/bin/env python3
"""Fail-closed verifier for the current repository content and paper."""
...
ALLOWED_PATHS = {
    ...
    "companion/affine_hjelmslev_radon_depth3.py",
    "companion/formulas.json",
    "companion/cross_chart_p3/README.md",
    "companion/cross_chart_p3/RESULT.json",
    "companion/cross_chart_p3/VERIFICATION_RECEIPT.json",
    "companion/cross_chart_p3/cross_chart_certificate.py",
    "companion/cross_chart_p3/verify_cross_chart_p3.py",
    "companion/epsilon1_p2/README.md",
    "companion/epsilon1_p2/RESULT.json",
    "companion/epsilon1_p2/VERIFICATION_RECEIPT.json",
    "companion/epsilon1_p2/epsilon1_p2_certificate.py",
    "companion/epsilon1_p2/verify_epsilon1_p2.py",
    "companion/exceptional_profiles/README.md",
    "companion/exceptional_profiles/INPUT.json",
    "companion/exceptional_profiles/EXPECTED.json",
    "companion/exceptional_profiles/RESULT.json",
    "companion/exceptional_profiles/VERIFICATION_RECEIPT.json",
    "companion/exceptional_profiles/O1_RESULT.json",
    "companion/exceptional_profiles/O1_VERIFICATION_RECEIPT.json",
    "companion/exceptional_profiles/depth3_smith.py",
    "companion/exceptional_profiles/o1_exceptional_certificate.py",
    "companion/exceptional_profiles/verify_exceptional_profiles.py",
    "companion/exceptional_profiles/verify_o1_exceptional.py",
    "docs/PUBLIC_CLAIM_BOUNDARY.md",
    "docs/REPRODUCIBILITY_BOUNDARY.md",
    "docs/SOURCE_AND_ATTRIBUTION.md",
    "paper/latexmkrc",
    "paper/main.tex",
    "paper/generated/formulas.tex",
    ...
    "scripts/verify.py",
    "tests/test_depth3_smith_profiles.py",
...
EXPECTED_BIBLIOGRAPHY_SHA256 = (
    "BF3DC655ACA9256BFE9830B11D276D9ABDD0EA440C44C0D97BA0A314B4A32CAF"
)
...
GENERIC_HYGIENE_PATTERNS = (
    ...
    (
        "private workspace-prefixed filename",
        re.compile(r"(?i)\bp[0-9]{2,4}_[a-z0-9_]+\.(?:py|bib|tex)\b"),
    ),
)
RESTRICTED_PYTHON_PATHS = {
    "scripts/check_manifest.py",
    "scripts/check_release.py",
    "scripts/git_boundary.py",
    "scripts/runtime_bootstrap.py",
...
PUBLIC_REPOSITORY_URL = (
    ...
    "FCIG-Depth-Three-Smith-Profiles-and-Newton-Minor-Geometry-for-"
    "Affine-Hjelmslev-Radon-Incidence"
)
PUBLIC_AUTHOR_NAME = "Oleksiy Babanskyy"
VISIBLE_PRODUCTION_CODE = re.compile(
...
UNVERIFIABLE_PROVENANCE_PATTERNS = (
    (
        "internal answer-blind provenance",
        re.compile(r"(?i)\banswer-blind\b"),
    ),
    (
        "internal separately-frozen provenance",
        re.compile(r"(?i)\bseparately\s+frozen\b"),
    ),
    (
        "unbacked factorization receipt claim",
        re.compile(
            r"(?i)remaining\s+five\s+factorizations\s+are\s+listed\s+"
            r"in\s+the\s+verification\s+receipt"
        ),
    ),
)
PINNED_ACTION_REFS = {
...
def _scan_python_source(
    text: str, relative: str, *, restricted_language: bool,
) -> None:
    """Scan supported static literals and the restricted verifier language."""
    ...
    for node in ast.walk(syntax):
        ...
        if restricted_language and isinstance(node, ast.Call):
            ...
            if call_name in forbidden:
                raise AssertionError(
                    ...
                    f"{call_name} in restricted verifier file {relative}"
                )
    ...
    for relative in sorted(ALLOWED_PATHS):
        ...
        if path.suffix.lower() == ".py":
            _scan_python_source(
                text,
                relative,
                restricted_language=relative in RESTRICTED_PYTHON_PATHS,
            )
...
def _check_tex_source(root: Path = ROOT) -> None:
    ...
    for label, pattern in UNVERIFIABLE_PROVENANCE_PATTERNS:
        match = pattern.search(manuscript)
        if match:
            line = manuscript.count("\n", 0, match.start()) + 1
            raise AssertionError(f"{label} on manuscript line {line}")
...
def _check_scientific_locks(root: Path = ROOT) -> None:
    ...
    if re.search(r"(?:FIG|TAB)-[0-9]+", manuscript):
        raise AssertionError("visible private figure/table code in manuscript")
    ...
    if "\\cite{LV14,KOH09,HVM89}" not in manuscript:
        raise AssertionError("adjacent projective-source citation drift")
    ...
    proof_typing_locks = (
        ...
        "\\cite[Theorem~8.3, equation~(26)]{BAB26D2}",
    )
    for lock in proof_typing_locks:
        if lock not in manuscript:
            raise AssertionError(f"divided-transfer typing drift: {lock}")
    if "B_3=\\left(\\begin{smallmatrix}B_2^\\transpose" in manuscript:
        raise AssertionError("incidence transpose orientation drift")

    proof_completion_locks = (
        "Sections~\\ref{sec:sector}--\\ref{sec:filtered}",
        "modulo each monic factor",
        "the column $(i,k)$ in (4.8)",
        "\\tag{8.15a}",
        "\\mathsf P(A,B;J,S)",
        "J\\ne S,\\ J\\ne0",
        "J=S=0",
    ...
    for lock in proof_completion_locks:
        if lock not in manuscript:
            raise AssertionError(f"public proof completion drift: {lock}")

    citation = native_path(root / "CITATION.cff").read_text(encoding="utf-8")
    citation_locks = (
        'title: "Affine Hjelmslev--Radon Depth-Three Smith Companion"',
        "type: software",
        'version: "1.0.0"',
    ...
    workflow_locks = (
        ...
        "Optimized pytest compatibility only; not a verification layer",
    ...
    companion = native_path(
        root / "companion" / "affine_hjelmslev_radon_depth3.py"
    ).read_text(encoding="utf-8")
    locks = (
        'SCHEMA = "fcig.depth3.formulas.v1"',
        "def newton_data(p: int)",
        "def validate_profile_identities(data:",
        '"schema": "fcig.depth3.companion-receipt.v1"',
    )
    for lock in locks:
        if lock not in companion:
            raise AssertionError(f"companion verification lock missing: {lock}")
...
def _scan_pdf_bytes(
    payload: bytes, *, label: str, scan_printable_text: bool = True,
) -> None:
...
def _check_terminal_pdf_bytes(payload: bytes) -> None:
    ...
    # Compressed stream bytes are arbitrary and can contain accidental
    # printable substrings. Raw bytes are checked structurally above; textual
    # hygiene is applied after every stream is decoded in ``inspect_pdf``.
    _scan_pdf_bytes(
        payload, label="complete raw PDF", scan_printable_text=False,
    )
...
def inspect_pdf(path: Path) -> dict[str, object]:
    ...
    full_text = "\n".join(page_text)
    match = VISIBLE_PRODUCTION_CODE.search(full_text)
    if match:
        raise AssertionError(f"visible private production code in PDF: {match.group(0)}")
    author_pages = [
        page_number
...
def inspect_git_history(root: Path = ROOT) -> dict[str, object]:
    """Bind verification to a clean checkout of the current HEAD.

    The content check is intentionally independent of branching policy: normal
    descendants, merges, tags, remotes and detached CI checkouts are all valid.
...
def main() -> int:
    ...
    print(
        ...
        f"git_commits={result['git']['commits']}"
    )
    return 0
